...
export default defineComponent({
  ...
  setup() {
    ...
    const updateUser = () => {
      ...
      // user ---> 代理对象，obj ---> 目标对象
      // user 对象或者 obj 对象添加一个新的属性，哪一种方式会影响界面更新
      // obj.gender = '男' // 这种方式，界面没有更新渲染
      // user.gender = '男' // 这种方式，界面可以更新渲染，而且这个数据最终也添加到了 obj 对象上

      // user 对象或者 obj 对象中移除一个已经存在的属性，哪一种方式会影响界面的更新
      // delete obj.age // 界面没有更新渲染，obj 中确实没有了 age 这个属性
      // delete user.age // 界面更新渲染了，obj 中确实没有了 age 这个属性

      // 总结：如果操作代理对象，目标对象中的数据也会随之变化，同时如果想要在操作数据的时候，界面也要跟着重新渲染，那么也是操作代理对象
